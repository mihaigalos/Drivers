...
/*
*   debug:          if true, debug messages are outout via the Serial interface
* (USB)
*   itgAddress:     specifies the I2C address of the sensor
*   selfHeating:    if so desired, users may specify the amount of self heating
* of the sensor to calibrate the temperature reading
*/
ITG3200::ITG3200(bool debug, char itgAddress, float selfHeatingAmount) {
  ...
  itgWrite(itgAddress_, DLPF_FS,
           (DLPF_FS_SEL_0 | DLPF_FS_SEL_1 |
            DLPF_CFG_0)); // scale for the outputs to +/-2000 degrees per second
...
vector<int> ITG3200::getRotations() {
  ...
  yRate = readY();
  zRate = readZ();
  result.push_back(xRate);
  result.push_back(yRate);
  result.push_back(zRate);
  if (debug_) {
    Serial.print(String(xRate) + String('\t') + String(yRate) + String('\t') +
                 String(zRate) + String("\t\t")); // Print the output rates to
                                                  // the terminal, seperated by
                                                  // a TAB character.
  }
  return result;
}
...
float ITG3200::getTemperature() {
  ...
  // from the datasheet : 35 degrees Celsius = (1/280) * -13200 + b; b =
  // 82.142857.
