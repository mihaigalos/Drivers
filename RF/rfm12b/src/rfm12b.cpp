...
// - networkid [optional - default = 170] (0-255 for RF12B, only 212 allowed for
// RF12)
...
void RFM12B::Initialize(uint8_t ID, uint8_t freqBand, uint8_t networkid,
                        uint8_t txPower, uint8_t airKbps,
                        uint8_t lowVoltageThreshold) {
  ...
  XFER(0x9850 | (txPower > 7 ? 7 : txPower)); // !mp,90kHz,MAX OUT
                                              // //last byte=power level:
                                              // 0=highest, 7=lowest
...
void RFM12B::TxRx() {
  ...
  if (rxstate == TXRECV) {
    ...
    do {
      ...
      dbg2(rf12_crc >> 8, HEX);
      dbg("_");
      dbg2((uint8_t)rf12_crc, HEX);
      if ((rxfill == 0) && (networkID != 0)) {
        // rf12_buf[rxfill] = networkID;
      } else if (TRANSMISSION_HEADER_LENGTH ==
                 rxfill) { // length is within the header
    ...
    dbg("Got CRC: ");
    dbg2((uint8_t)(receivedCrc >> 8), HEX);
    dbg("_");
    dbg2((uint8_t)receivedCrc, HEX);
    dbg("   ");
    dbg("Computed CRC: ");
    dbg2((uint8_t)(rf12_crc >> 8), HEX);
    dbg("_");
    dbg2((uint8_t)rf12_crc, HEX);
    dbg("   ");

#ifndef COMPILE_FOR_AVR
    if (receivedCrc == rf12_crc)
      LedToggle();
#endif
    ...
    while (TXDONE != rxstate) {
      // AA AA AA (pramble)   2D  <Network Id>    <Destination>     <Source>
      // <Length>     <Payload>    <CRC1>    <CRC2>
...
bool RFM12B::ReceiveComplete() {
  if (rxstate == TXRECV && (rxfill >= rf12_len + 6 || rxfill >= RF_MAX)) {
    ...
    if (RF12_DESTID == 0 ||
        RF12_DESTID == ownId) { // if (!(rf12_hdr & RF12_HDR_DST) || (nodeID &
                                // NODE_ID) == 31 || (rf12_hdr & RF12_HDR_MASK)
                                // == (nodeID & NODE_ID)) {
...
void RFM12B::SendStart(uint8_t toNodeID, const void *sendBuf, uint8_t sendLen,
...
void RFM12B::Send(uint8_t toNodeID, const void *sendBuf, uint8_t sendLen,
                  bool requestACK) {
  while (!CanSend())
    ;
...
bool RFM12B::LowBattery() { return (Control(0x0000) & RF_LBD_BIT) != 0; }

uint8_t RFM12B::GetSender() { return RF12_SOURCEID; }
...
bool RFM12B::ACKReceived(uint8_t fromNodeID) {
  if (ReceiveComplete())
    return CRCPass() && RF12_DESTID == ownId &&
...
#define MX                                                                     \
  (((z >> 5 ^ y << 2) + (y >> 3 ^ z << 4)) ^                                   \
   ((sum ^ y) + (cryptKey[(uint8_t)((p & 3) ^ e)] ^ z)))
