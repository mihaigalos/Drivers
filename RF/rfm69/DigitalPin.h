...
fastBitWriteSafe(volatile uint8_t *address, uint8_t bit, bool level) {
  uint8_t oldSREG;
 ...
 */
static inline __attribute__((always_inline)) void
fastPinConfig(uint8_t pin, bool mode, bool level) {
...
public:
   ...
   *                  If mode is input, enable or disable the pin's 20K pullup.
   */
  inline __attribute__((always_inline)) void config(bool mode, bool level) {
