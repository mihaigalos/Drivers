...
volatile uint8_t RFM69::ACK_RECEIVED; // should be polled immediately after
                                      // sending a packet with ACK request
volatile int16_t RFM69::RSSI; // most accurate RSSI during reception (closest to
                              // the reception)
...
bool RFM69::initialize(uint8_t freqBand, uint8_t nodeID, uint8_t networkID) {
  const uint8_t CONFIG[][2] = {
      ...
      /* 0x05 */ {REG_FDEVMSB, RF_FDEVMSB_50000}, // default: 5KHz, (FDEV +
                                                  // BitRate / 2 <= 500KHz)
      /* 0x06 */ {REG_FDEVLSB, RF_FDEVLSB_50000},

      /* 0x07 */ {REG_FRFMSB, (uint8_t)(freqBand == RF69_315MHZ
                                            ? RF_FRFMSB_315
                                            : (freqBand == RF69_433MHZ
                                                   ...
                                                   : (freqBand == RF69_868MHZ
                                                          ? RF_FRFMSB_868
                                                          : RF_FRFMSB_915)))},
      /* 0x08 */ {REG_FRFMID, (uint8_t)(freqBand == RF69_315MHZ
                                            ? RF_FRFMID_315
                                            : (freqBand == RF69_433MHZ
                                                   ? RF_FRFMID_433
                                                   : (freqBand == RF69_868MHZ
                                                          ? RF_FRFMID_868
                                                          : RF_FRFMID_915)))},
      /* 0x09 */ {REG_FRFLSB, (uint8_t)(freqBand == RF69_315MHZ
                                            ? RF_FRFLSB_315
                                            : (freqBand == RF69_433MHZ
                                                   ? RF_FRFLSB_433
                                                   : (freqBand == RF69_868MHZ
      ...
      // for BR-19200: /* 0x19 */ { REG_RXBW, RF_RXBW_DCCFREQ_010 |
      // RF_RXBW_MANT_24 | RF_RXBW_EXP_3 },
      /* 0x25 */ {REG_DIOMAPPING1,
                  RF_DIOMAPPING1_DIO0_01}, // DIO0 is the only IRQ we're using
      ...
      /* 0x2F */ {REG_SYNCVALUE1, 0x2D}, // attempt to make this compatible with
                                         // sync1 byte of RFM12B lib
      ...
      /* 0x3D */ {REG_PACKETCONFIG2,
                  RF_PACKET2_RXRESTARTDELAY_2BITS |
                      ...
                      RF_PACKET2_AES_OFF}, // RXRESTARTDELAY must match
                                           // transmitter PA ramp-down time
                                           // (bitrate dependent)
      // for BR-19200: /* 0x3D */ { REG_PACKETCONFIG2,
      ...
      /* 0x6F */ {REG_TESTDAGC,
                  RF_DAGC_IMPROVED_LOWBETA0}, // run DAGC continuously in RX
                                              // mode for Fading Margin
...
// just call receiveDone()
void RFM69::sleep() { setMode(RF69_MODE_SLEEP); }
...
void RFM69::setCallback(TCallback_void_void callback) { _callback = callback; }
...
// lower RSSI at the receiver
// the power configurations are explained in the SX1231H datasheet (Table 10 on
// p21; RegPaLevel p66): http://www.semtech.com/images/datasheet/sx1231h.pdf
// valid powerLevel parameter values are 0-31 and result in a directly
// proportional effect on the output/transmission power
...
bool RFM69::canSend() {
  if (_mode == RF69_MODE_RX && PAYLOADLEN == 0 &&
      readRSSI() < CSMA_LIMIT) // if signal stronger than -100dBm is detected
                               // assume channel activity
  {
    setMode(RF69_MODE_STANDBY);
    return true;
  }
...
// The only twist is that you have to manually listen to ACK requests on the
// other side and send back the ACKs
...
void RFM69::sendFrame(uint8_t toAddress, const void *buffer, uint8_t bufferSize,
  ...
  setMode(RF69_MODE_STANDBY); // turn off receiver to prevent reception while
                              // filling fifo
  while ((readReg(REG_IRQFLAGS1) & RF_IRQFLAGS1_MODEREADY) == 0x00)
    ;                                                // wait for ModeReady
  ...
  // while (readReg(REG_IRQFLAGS2) & RF_IRQFLAGS2_PACKETSENT == 0x00); // wait
  // for ModeReady
  setMode(RF69_MODE_STANDBY);
}
...
void RFM69::interruptHandler() {
  ...
  if (_mode == RF69_MODE_RX &&
    ...
    if (!(_promiscuousMode || TARGETID == _address ||
          TARGETID == RF69_BROADCAST_ADDR) // match this node's address, or
                                           // broadcast address or anything in
                                           // promiscuous mode
        || PAYLOADLEN < 3) // address situation could receive packets that are
                           // malformed and don't fit this libraries extra
                           // fields
    {
      PAYLOADLEN = 0;
      unselect();
...
void RFM69::receiveBegin() {
  ...
  while (0 == digitalRead(_interruptPin))
    ; // simulate interrupt
...
void RFM69::promiscuous(bool onOff) {
  ...
  // writeReg(REG_PACKETCONFIG1, (readReg(REG_PACKETCONFIG1) & 0xF9) | (onOff ?
  // RF_PACKET1_ADRSFILTERING_OFF : RF_PACKET1_ADRSFILTERING_NODEBROADCAST));
}

// for RFM69HW only: you must call setHighPower(true) after initialize() or else
...
void RFM69::setHighPower(bool onOff) {
  ...
  if (_isRFM69HW) // turning ON
    ...
    writeReg(REG_PALEVEL, RF_PALEVEL_PA0_ON | RF_PALEVEL_PA1_OFF |
                              RF_PALEVEL_PA2_OFF |
