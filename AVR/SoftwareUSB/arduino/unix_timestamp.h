...
// Mihai Galos 11.05.2018: commenting out certain months' ends. This reduces the column count of the
// preprocess'd / generated cpp to below max (65535).

#define GET_MONTH(str, i)      (str[i]=='J' && str[i+1]=='a' ? 1 :     \
                                str[i]=='F' ? 2 :     \
...
// approximate daylight saving from ~last Sunday in March to ~the last Sunday in October
#define LOCATION_DAYLIGHT_SAVING (__TIME_MONTH__>3 && __TIME_MONTH__<11 ? 1:0)

#define LOCAL_TIME (__TIME_HOURS__ - TIME_ZONE - LOCATION_DAYLIGHT_SAVING)
//#define __LOCATION_HOURS__  (LOCAL_TIME>=0 ? LOCAL_TIME : LOCAL_TIME+24)
...
#define _UNIX_TIMESTAMP_YDAY(year, month, day) \
    ( \
        day \
        + (month >=  2 ? 31UL : 0UL) \
        + (month >=  3 ? _UNIX_TIMESTAMP_FDAY(year) : 0UL) \
        + (month >=  4 ? 31UL : 0UL) \
        + (month >=  5 ? 30UL : 0UL) \
        + (month >=  6 ? 31UL : 0UL) \
        + (month >=  7 ? 30UL : 0UL) \
